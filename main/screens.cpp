...
void drawMainScreen() {
  tft.fillScreen(TFT_WHITE); // Limpa a tela inteira com branco primeiro. ESSENCIAL!
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    tft.drawString(dateString, time_x, time_y + tft.fontHeight(1) + 2, 1);
  }

  // Retângulo superior para o título "PHARMALOG"
  // Esta área já é limpa pelo tft.fillScreen(TFT_WHITE) no início.
  // Podemos apenas desenhar o título diretamente.
  tft.setTextSize(2);
  tft.setTextColor(TFT_BLACK);
  // Posição ajustada para ser centralizada verticalmente na área que o título ocuparia
  int center_y_title_area = (60 + 110) / 2; // Meio entre Y=60 e Y=110
  tft.drawCentreString("PHARMALOG", SCREEN_WIDTH / 2, center_y_title_area - tft.fontHeight(2)/2, 2);

  // Área para Temperatura e Umidade
  tft.fillRect(0, 110, SCREEN_WIDTH, 75, TFT_WHITE); // Fundo para a área de T/U
  tft.setTextSize(FONT_SIZE_LARGE);
  tft.setTextColor(TFT_BLUE); // Cor do texto da temperatura e umidade - MANTIDO TFT_BLUE CONFORME SUA ÚLTIMA VERSÃO.
  ...
  tft.drawCentreString("Temp.: " + tempDisplay, SCREEN_WIDTH / 2, 60 + text_offset_y, 2); // MANTIDO Y=60, e tamanho 2.

  String humDisplay = isnan(humidity) ? "XX %" : String(humidity, 0) + " %";
  tft.drawCentreString("Umid.: " + humDisplay, SCREEN_WIDTH / 2, 90 + text_offset_y + tft.fontHeight(FONT_SIZE_LARGE) + 5, 2); // MANTIDO Y=90, e tamanho 2.


  // Área para o status Wi-Fi e MQTT
  tft.fillRect(0, SCREEN_HEIGHT - 60, SCREEN_WIDTH, 40, TFT_WHITE); // Limpa a área
  
  String wifiInfo; // Declaração adicionada aqui

  // Status Wi-Fi
  String wifiStatusText = "WiFi: ";
  uint16_t wifiStatusColor = TFT_LIGHTGREY;
  switch(currentWifiStatus) {
    case WIFI_IDLE:
      wifiStatusText += "Desconectado";
      wifiStatusColor = TFT_RED;
      break;
    case WIFI_CONNECTING:
      wifiStatusText += "Conectando...";
      wifiStatusColor = TFT_YELLOW;
      break;
    case WIFI_CONNECTED_OK:
      wifiStatusText += "Conectado";
  ...
  // Se WiFi estiver conectado pelo próprio módulo, força status para OK (para status fora deste controle)
  if (WiFi.status() == WL_CONNECTED && currentWifiStatus != WIFI_CONNECTED_OK) {
    currentWifiStatus = WIFI_CONNECTED_OK;
  } else if (WiFi.status() != WL_CONNECTED && currentWifiStatus == WIFI_CONNECTED_OK) {
     // Se desconectou, volta para IDLE
     currentWifiStatus = WIFI_IDLE;
  }

  if (WiFi.status() == WL_CONNECTED) {
    wifiInfo = "IP: " + WiFi.localIP().toString();
    wifiInfo += " | SSID: " + WiFi.SSID();
    tft.setTextColor(TFT_DARKGREEN, TFT_WHITE); // Texto verde escuro para IP/SSID
  } else {
    wifiInfo = wifiStatusText; // Exibe o status "Desconectado", "Conectando", "Falha"
    // Cor já definida no switch acima.
    tft.setTextColor(wifiStatusColor, TFT_WHITE);
  }
  tft.setTextSize(1);
  tft.drawCentreString(wifiInfo, SCREEN_WIDTH / 2, SCREEN_HEIGHT - 40, 1);

  // Status MQTT
  String mqttStatusText = "MQTT: ";
  uint16_t mqttStatusColor = TFT_LIGHTGREY;

  switch (currentMqttStatus) {
    case APP_MQTT_CONNECTED:
      mqttStatusText += "Conectado";
      mqttStatusColor = TFT_GREEN;
      break;
    case APP_MQTT_DISCONNECTED:
      mqttStatusText += "Desconectado";
      mqttStatusColor = TFT_RED;
      break;
    case APP_MQTT_CONNECTING:
      mqttStatusText += "Conectando...";
      mqttStatusColor = TFT_YELLOW;
      break;
  }
  // Desenha o status MQTT abaixo do status Wi-Fi
  tft.setTextColor(mqttStatusColor, TFT_WHITE);
  tft.drawCentreString(mqttStatusText, SCREEN_WIDTH / 2, SCREEN_HEIGHT - 25, 1);

  drawButton(menuButton); // Desenha o botão MENU por último, sobre tudo
}
...
void drawMqttConfigDisplayScreen() {
  ...
  tft.setTextSize(1);
  tft.setTextColor(TFT_WHITE);
  tft.drawString("Usuario:", text_start_x, current_y, 1); // Título do campo
  current_y += tft.fontHeight(1) + 2; // Espaçamento entre título e caixa de texto
  ...
  tft.setTextColor(TFT_BLACK, TFT_WHITE);
  tft.setTextSize(FONT_SIZE_VALUE);
  String mqttUsernameDisplay = String(storedMqttUsername); // LÊ DA VARIÁVEL GLOBAL
  ...
  tft.setTextSize(1);
  tft.setTextColor(TFT_WHITE);
  tft.drawString("Senha:", text_start_x, current_y, 1); // Título do campo
  current_y += tft.fontHeight(1) + 2; // Espaçamento entre título e caixa de texto
  ...
  tft.setTextColor(TFT_BLACK, TFT_WHITE);
  tft.setTextSize(FONT_SIZE_VALUE);
  String mqttPasswordDisplay = String(storedMqttPassword); // LÊ DA VARIÁVEL GLOBAL
  // Lógica de truncagem (para o caso de a senha não ser mascarada)
  String tempMqttPassword = "";
  for (char c : mqttPasswordDisplay) {
  ...
  if (tempMqttPassword.length() < mqttPasswordDisplay.length()) { mqttPasswordDisplay = tempMqttPassword + "..."; } else { mqttPasswordDisplay = tempMqttPassword; }
  ...
  // --- Campo Chave API ---
  tft.setTextSize(1); // Título do campo
  tft.setTextColor(TFT_WHITE);
  ...
  current_y += TEXT_INPUT_HEIGHT + 10; // Espaçamento antes do botão de teste (PADRONIZADO)

  // --- Botão Testar Conexão MQTT ---
  testMqttConnectionButton.x = (SCREEN_WIDTH / 2) - (testMqttConnectionButton.width / 2);
  testMqttConnectionButton.y = current_y; // Posição Y calculada
  drawButton(testMqttConnectionButton);

  // O botão Voltar (backButtonMqttConfig) já é desenhado no início da função
}
...
void drawTextInputScreen(const char* title, const String& currentText) {
  tft.fillScreen(TFT_DARKCYAN); // Limpa a tela completamente ao entrar neste menu
...
void drawWifiConnectionStatusScreen(const char* message, const char* ssid, uint16_t color) {
  tft.fillScreen(TFT_BLACK); // Limpa a tela completamente ao entrar neste menu
