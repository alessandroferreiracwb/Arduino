...
bool isRemoteMode = false; // Novo: Variável para controlar o modo
...
Screen currentScreen = MAIN_SCREEN; // Alteração: Começa na tela principal
...
void setup() {
    ...
    ts.setRotation(1);
    loadConfig();
    // A chamada setupWifi() foi removida daqui, pois a conexão será iniciada pelo usuário.
    drawMainScreen();
}
...
void loop() {
    if (isRemoteMode) {
        ...
        // Lógica para ler e processar a CAN nativa do ESP32 aqui (no modo LOCAL)
...
void checkWifiConnection() {
    ...
    if (isConnected) {
        if (!client.connected()) {
            ...
            if (currentScreen == MAIN_SCREEN) {
                drawMainScreen(); // Atualiza o status na tela principal
            }
        }
        return;
    }
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        WiFi.begin(ssid, password);
        if (currentScreen == MAIN_SCREEN) {
            drawMainScreen(); // Atualiza o status "Conectando..."
        }
        return;
    }
...
void drawConnectingScreen() {
    // Esta tela agora só será usada em caso de erro, mas a lógica de conexão
    // em segundo plano na main screen a substitui para a maioria dos casos.
    tft.fillScreen(TFT_BLACK);
    tft.setTextSize(2);
...
void drawMainScreen() {
    ...
    // Novo: Desenha o status da conexão Wi-Fi no canto superior esquerdo
    tft.setTextSize(1);
...
void handleTouch() {
    if (ts.touched()) {
        ...
        if (currentScreen == MAIN_SCREEN) {
            ...
            // Lógica para os botões LOCAL e REMOTO
            if (touch_x > 10 && touch_x < 110 && touch_y > 200 && touch_y < 230) {
                isRemoteMode = false;
                if (isConnected) {
                    client.stop(); // Encerra a conexão TCP com o servidor
                    WiFi.disconnect(); // Desconecta do Wi-Fi
                    isConnected = false;
                }
                drawSetupScreen();
            } else if (touch_x > 120 && touch_x < 220 && touch_y > 200 && touch_y < 230) {
                isRemoteMode = true;
                setupWifi(); // Inicia a conexão Wi-Fi
                drawSetupScreen();
            }
            // Lógica para os outros botões da tela de Setup
...
void saveConfig() {
    ...
    preferences.putBool("isRemoteMode", isRemoteMode); // Novo: Salva o estado do modo
    preferences.end();
...
void loadConfig() {
    ...
    isRemoteMode = preferences.getBool("isRemoteMode", false); // Novo: Carrega o estado do modo
    preferences.end();
