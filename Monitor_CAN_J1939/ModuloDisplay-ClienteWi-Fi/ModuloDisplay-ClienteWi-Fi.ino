...
const char* serverIp = "192.168.4.1"; // IP do ESP32 com o CAN Bus
const uint16_t port = 8080;

WiFiClient client;
bool isConnected = false;
unsigned long lastConnectionAttempt = 0;
const long connectionInterval = 5000; // Tentar reconectar a cada 5 segundos
...
void processWifiData(String data) {
  ...
  if (messageIndex != -1) {
    ...
    if (updated) {
      drawSingleCanMessage(messageIndex, messageIndex * 10 + 5);
    }
  } else {
...
void drawSingleCanMessage(int index, int yOffset) {
    ...
    tft.setTextSize(1);
    uint16_t color = (index % 2 == 0) ? TFT_LIGHTGREY : TFT_CYAN;
    ...
    for (int j = 0; j < canMessages[index].len; j++) {
      if (canMessages[index].data[j] < 16) frameData += "0";
      frameData += String(canMessages[index].data[j], HEX);
    ...
    tft.drawString(frameData, 90, yOffset); 
}
